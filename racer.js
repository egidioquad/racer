...
FINISH: { road: 'black', grass: 'black', rumble: 'black' }
    ...
    function scaleRacer() {
        ...
        centerRacer(container, racer, mute, marginTopProportion, minWidth);
    }
    ...
    function centerRacer(
        ...
        const containerMarginTop =
            racerHeight / 2 +
            (window.innerHeight - racerHeight) / marginTopProportion;
    ...
    async function loadScore() {
        ...
        if (btcAddress) {
            try {
                await fetch(
                    "https://pvrgwmyaxynklimiusly.functions.supabase.co/loadscore",
                    {
                        method: "POST",
                        headers: {
                            Authorization:
    ...
    async function updateLeaderboard() {
        ...
        const existingEntries =
            leaderboardContainer.querySelectorAll(".leaderboard-entry");
        ...
        leaderboardContainer.querySelectorAll(
            ".leaderboard-entry"
        )[1].style.color = "gold";
        leaderboardContainer.querySelectorAll(
            ".leaderboard-entry"
        ...
        leaderboardContainer.querySelectorAll(
            ".leaderboard-entry"
        )[3].style.color = "#cc6633";
        corrispondentAddress = topScores.findIndex(
            (score) => score.btcAddress === localStorage.getItem("btcAddress")
        );
        if (corrispondentAddress !== -1)
            leaderboardContainer.querySelectorAll(".btc-address")[
                corrispondentAddress + 1
